...
public class GameEngine implements Runnable{
	...
	public void init() {
		...
		if(config.containsKey("FRAME_TITLE")) {
			window.getFrame().setTitle((String)config.get("FRAME_TITLE"));
		}
		...
		window.getFrame().addKeyListener(new KeyListener() {
			...
			public void keyReleased(KeyEvent e) {
				...
				if(config.containsKey("EXIT_KEY")) {
					if(e.getKeyCode() == (int)config.get("EXIT_KEY")) {
						stop();
					}
				}
			}
	...
	public void postInit() {
		...
		if(config.containsKey("IMG_WIDTH")) {
			w = (int)config.get("IMG_WIDTH");
			w = w < MAX_WIDTH ? w : MAX_WIDTH;
		}
		if(config.containsKey("IMG_HEIGHT")) {
			h = (int)config.get("IMG_HEIGHT");
