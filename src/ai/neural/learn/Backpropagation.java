...
public class Backpropagation extends DeltaRule {
	...
	public Double calcDeltaWeight(int layer, int input, int neuron) {
		...
		switch (learningMode) {
		case BATCH:
			deltaWeight *= LearningRate / ((double) (trainingDataSet.numberOfRecords));
		case ONLINE:
	...
	public void test(int i) throws NeuralException {
		...
		for (int j = 0; j < neuralNet.getNumberOfOutputs(); j++) {
			...
			testingError.get(i).set(j, simpleError(testingDataSet.getIthTargetOutputArrayList(j, normalization).get(i),
