...
public class Hebbian extends LearningAlgorithm {
	...
	public Hebbian(NeuralNet _neuralNet, NeuralDataSet _trainDataSet, LearningMode _learningMode) {
	...
	public Double calcNewWeight(int layer, int input, int neuron) throws NeuralException {
		if (layer > 0) {
			...
			case BATCH:
				...
				for (int i = 0; i < trainingDataSet.numberOfRecords; i++) {
					multResultIthInput += trainingDataSet.getArrayNeuralOutputRecord(i).get(neuron) * _ithInput.get(i);
				}
	...
	public Double calcNewWeight(int layer, int input, int neuron, double error) throws NeuralException {
		throw new NeuralException(
				"Hebbian learning can be used only with the " + "neuron's inputs and outputs, no error is used");
	...
	public void print() {
		if (learningMode == LearningMode.ONLINE)
			System.out.println("Epoch=" + String.valueOf(epoch) + "; Record=" + String.valueOf(currentRecord));
		else
