...
package ai.neural.learn;

import java.util.ArrayList;

import ai.neural.HiddenLayer;
import ai.neural.NeuralException;
import ai.neural.NeuralNet;
import ai.neural.Neuron;
import ai.neural.OutputLayer;
...
public class DeltaRule extends LearningAlgorithm {
	...
	public DeltaRule(NeuralNet _neuralNet, NeuralDataSet _trainDataSet, LearningMode _learningMode) {
	...
	public void forward(int i) throws NeuralException {
		if (neuralNet.getNumberOfHiddenLayers() > 0) {
			...
			generalError.set(i, generalError(trainingDataSet.getArrayTargetOutputRecord(i, normalization),
					trainingDataSet.getArrayNeuralOutputRecord(i, normalization)));
			for (int j = 0; j < neuralNet.getNumberOfOutputs(); j++) {
				overallError.set(j, overallError(trainingDataSet.getIthTargetOutputArrayList(j, normalization),
						trainingDataSet.getIthNeuralOutputArrayList(j, normalization)));
				error.get(i).set(j, simpleError(trainingDataSet.getIthTargetOutputArrayList(j, normalization).get(i),
						trainingDataSet.getIthNeuralOutputArrayList(j, normalization).get(i)));
			}
			overallGeneralError = overallGeneralErrorArrayList(trainingDataSet.getArrayTargetOutputData(normalization),
		...
		if (neuralNet.getNumberOfHiddenLayers() > 0) {
			...
			for (int i = 0; i < trainingDataSet.numberOfRecords; i++) {
				...
				generalError.set(i, generalError(trainingDataSet.getArrayTargetOutputRecord(i, normalization),
						trainingDataSet.getArrayNeuralOutputRecord(i, normalization)));
				for (int j = 0; j < neuralNet.getNumberOfOutputs(); j++) {
					error.get(i).set(j, simpleError(trainingDataSet.getArrayTargetOutputRecord(i, normalization).get(j),
							trainingDataSet.getArrayNeuralOutputRecord(i, normalization).get(j)));
				}
			}
			for (int j = 0; j < neuralNet.getNumberOfOutputs(); j++) {
				overallError.set(j, overallError(trainingDataSet.getIthTargetOutputArrayList(j, normalization),
	...
	public void test(int i) throws NeuralException {
		if (neuralNet.getNumberOfHiddenLayers() > 0) {
			...
			testingGeneralError.set(i, generalError(testingDataSet.getArrayTargetOutputRecord(i, normalization),
					testingDataSet.getArrayNeuralOutputRecord(i, normalization)));
			for (int j = 0; j < neuralNet.getNumberOfOutputs(); j++) {
				testingOverallError.set(j, overallError(testingDataSet.getIthTargetOutputArrayList(j, normalization),
						testingDataSet.getIthNeuralOutputArrayList(j, normalization)));
				testingError.get(i).set(j,
						simpleError(testingDataSet.getIthTargetOutputArrayList(j, normalization).get(i),
								testingDataSet.getIthNeuralOutputArrayList(j, normalization).get(i)));
			}
	...
	public void test() throws NeuralException {
		if (neuralNet.getNumberOfHiddenLayers() > 0) {
			...
			for (int i = 0; i < testingDataSet.numberOfRecords; i++) {
				...
				for (int j = 0; j < neuralNet.getNumberOfOutputs(); j++) {
					testingError.get(i).set(j,
							simpleError(testingDataSet.getArrayTargetOutputRecord(i, normalization).get(j),
									testingDataSet.getArrayNeuralOutputRecord(i, normalization).get(j)));
				}
			}
			for (int j = 0; j < neuralNet.getNumberOfOutputs(); j++) {
				testingOverallError.set(j, overallError(testingDataSet.getIthTargetOutputArrayList(j, normalization),
	...
	public void print() {
		...
		if (learningMode == LearningMode.ONLINE)
			msgPrint = "Epoch=" + String.valueOf(epoch) + "; Record=" + String.valueOf(currentRecord)
					+ "; Overall Error=" + String.valueOf(overallGeneralError);
		else
			msgPrint = "Epoch= " + String.valueOf(epoch) + "; Overall Error =" + String.valueOf(overallGeneralError);
