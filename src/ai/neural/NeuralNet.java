...
import ai.neural.init.WeightInitialization;
import ai.neural.math.IActivationFunction;
import java.util.ArrayList;

/**
 *
 * NeuralNet This class represents the Neural Network itself. It contains all
 * the definitions that a Neural Network has, including method for calculation
 * (forward).
 * 
...
public class NeuralNet {
	...
	/**
	 * NeuralNet constructor This constructor initializes the neural network by
	 * initializing all of the underlying layers and their respective neurons.
	 * 
	 ...
	 * @param numberofhiddenneurons Array containing the number of Neurons in each
	...
	public NeuralNet(int numberofinputs, int numberofoutputs, int[] numberofhiddenneurons,
		...
		for (int i = 0; i <= numberOfHiddenLayers; i++) {
			...
			if (i == 0) {
				...
				indexesWeightPerLayer[i] = indexesWeightPerLayer[i - 1] + (neuronsInHiddenLayers[i - 1]
						* ((i == 1 ? numberofinputs : neuronsInHiddenLayers[i - 2]) + 1));
			}
		}
		if (numberOfHiddenLayers > 0) {
			...
			indexesWeightPerLayer[numberOfHiddenLayers + 1] = indexesWeightPerLayer[numberOfHiddenLayers]
					+ neuronsInHiddenLayers[numberOfHiddenLayers] * (numberOfInputs + 1);
		}
		...
		if (numberOfHiddenLayers == hiddenAcFnc.length) {
			...
			for (int i = 0; i < numberOfHiddenLayers; i++) {
				if (i == 0) {
					try {
						hiddenLayer.set(i, new HiddenLayer(this, numberofhiddenneurons[i], hiddenAcFnc[i],
								inputLayer.getNumberOfNeuronsInLayer()));
					} catch (IndexOutOfBoundsException iobe) {
						hiddenLayer.add(new HiddenLayer(this, numberofhiddenneurons[i], hiddenAcFnc[i],
								inputLayer.getNumberOfNeuronsInLayer()));
					}
	...
	public NeuralNet(int numberofinputs, int numberofoutputs, int[] numberofhiddenneurons,
			IActivationFunction[] hiddenAcFnc, IActivationFunction outputAcFnc) {
		this(numberofinputs, numberofoutputs, numberofhiddenneurons, hiddenAcFnc, outputAcFnc,
	...
	/**
	 * setInputs Feeds an array of real values to the neural network's inputs
	 * 
	...
	/**
	 * setInputs Sets a vector of double-precision values into the neural network
	 * inputs
	 * 
	...
	/**
	 * calc This method calculates the output of each layer and forwards all values
	 * to the next layer
	 */
	public void calc() {
		...
		for (int l = 0; l <= numberOfHiddenLayers; l++) {
			...
			for (Neuron n : nl.getListOfNeurons()) {
				for (int i = 0; i <= n.getNumberOfInputs(); i++) {
					weights[indexesWeightPerLayer[l] + j * (neuronsInHiddenLayers[l] + 1) + i] = n.getWeight(i);
				}
				j++;
			}
		}
	...
	public int getTotalNumberOfWeights() {
		...
		result += outputLayer.numberOfNeuronsInLayer * (outputLayer.numberOfInputs + 1);
		return result;
	}
